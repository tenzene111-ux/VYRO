...
import { X, Radio, Globe2, Lock, Loader2 } from "lucide-react";
...
export function CreateChannel() {
  ...
  const handleCreate = async () => {
    ...
    setError(null);
    try {
      const channelId = await createChannel(name.trim(), description.trim(), privacy);
      navigate(`/chat/channel/${channelId}`, { replace: true });
    } catch (e) {
      setError(e instanceof Error ? e.message : "Couldn't create the channel. Try again.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="flex min-h-svh flex-col bg-vyro-radial px-5 safe-top">
      <header className="flex items-center justify-between py-4">
        ...
        <p className="font-display text-base font-semibold text-ink">Create Channel</p>
        <button
      ...
      <p className="mb-4 text-center text-[12px] text-mist">
        Channels are one-way broadcasts — only you (and admins you promote) can post. Everyone else can subscribe, react, and comment.
      </p>

      <div className="flex flex-col gap-3">
        <label className="flex flex-col gap-1.5">
          <span className="text-[11px] font-medium text-mist">Channel name</span>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. Daily Bhutan News"
      ...
      <div className="mt-4 flex flex-col gap-3">
        ...
        >
          ...
          <div>
            ...
            <p className="text-[12px] text-mist">Anyone can find and subscribe to this channel.</p>
          </div>
        </button>
        <button
        ...
        >
          ...
          <div>
            ...
            <p className="text-[12px] text-mist">Only visible to subscribers you invite.</p>
          </div>
        </button>
      </div>
